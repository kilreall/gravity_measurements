...
T = 2e-3 # ms
M = 0
Tg = 0.089# T1:0.4;T2:0.089;T4:0.0226;T6:0.0109;T8:0.0061;T10:0.00357;T12:0.0027;
gR = 9.87
Tf = 29210*1e-6
TF = Tf+2*T
...
r = 100000
# чтение csv
file_path = 'data\data_T_2_ms.csv' 
...
for i in range(data.shape[0]//101):
    ...
    if i != 0:
        M = 0
    m = round((w*gR+ph+np.pi/2)/2/np.pi) + M
    print(m)
    gcs = (-np.pi/2 + 2*np.pi*m-ph)/w
    if G ==[]:
        G.append(gcs)
    while abs(gcs - np.mean(np.array(G))) > abs((-np.pi/2 + 2*np.pi*(m+1)-ph)/w - np.mean(np.array(G))) or abs(gcs - np.mean(np.array(G))) > abs((-np.pi/2 + 2*np.pi*(m-1)-ph)/w - np.mean(np.array(G))):
        if abs(gcs - np.mean(np.array(G))) > abs((-np.pi/2 + 2*np.pi*(m+1)-ph)/w - np.mean(np.array(G))):
            m += 1
            gcs = (-np.pi/2 + 2*np.pi*m-ph)/w
        else: 
            m -= 1
    ...
    if i == 5 or i == 9: # Для T=2 i=5 и i=9
        print("выкинутый i", i)
