...
TF = Tf+2*T
ty = 15e-6 # s длительность pi/2 импульса
OR = np.pi/2/ty
r = 100000
...
for i in range(len(mdata[1])): # коррекция скорости чирпирования
    ...
    v0 = np.random.normal(loc=0, scale=1e-7)
    v = [v0]
    for j in range(len(a)):
        v.append(v[-1]+a[j]*(ta[-1]-ta[0])/(len(ta)-1))
    v = np.array(v) 
    ...
    t1, t2 = np.argmin(np.abs(60e-6*i-ta)), np.argmin(np.abs(60e-6*(i+1)-ta))
    fvibtest = simps(v[t1:t2], ta[t1:t2])
    print(t1, t2)
    #tidata[i] = np.sin(k*(gR-2*np.pi*mdata[1, i])*T**2+fvibtest)
    print(fvibtest/T/T/k*1e5)
    #break
    # fat = vfunc(ta)
    # intvib = fat*a
