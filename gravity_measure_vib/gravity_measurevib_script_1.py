...
for el in mdata[1]: # коррекция скорости чирпирования
    ta = adata[0]
    a =  adata[1]
    fat = fa(ta)
    intvib = fat*a
    fvib = simps(intvib, ta)
    el = el - k*fvib/T**2

mdata = np.vstack((mdata, mdata[2]*2*np.pi/k)) # ускорение geff
...
for i in range(data.shape[0]//n):
    idata = np.vstack((idata, data[:,1][n*i:n*(i+1)])) # присоединение прохода

    # fit каждого прохода
    initial_guess = [3000, 2*np.pi/Tg, 0, np.mean(mdata[i+3])] 
    par, cov = curve_fit(sins, mdata[2], idata[i], p0=initial_guess)
    A, w, ph, s = par
    sg = sign(A)
...
plt.figure(1, (10,6))
plt.scatter(mdata[1]*1e-6, ndata, color="blue")
plt.plot(mdata[1]*1e-6, ndata, color="blue", lw =1)
plt.plot(mdata[1]*1e-6, A*np.sin(w*mdata[2]+ph) + s, color="red", ls="--", lw=1.5)
print("signal snr" , A/np.std(ndata-A*np.sin(w*mdata[2]+ph) + s, ddof=1))
