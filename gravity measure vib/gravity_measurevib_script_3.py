...
## чтение csv a(t)
# file_path = 'adata\data_T_12_ms.csv' 
# adata = np.genfromtxt(file_path, delimiter=',', names=True, dtype=None)
# adata = np.array(adata.tolist())


## создание многомерного массива данных
mdata = data[:,0]/70*5282 # конечная частота рамановского излучения
...
for i in range(len(mdata[1])): # коррекция скорости чирпирования
    ...
    a = np.random.normal(loc=0.0, scale=10*1e-9, size=54000)
    v0 = float(np.random.normal(loc=0.0, scale=10*1e-9, size=1))
    v = [v0]
    ...
    tidata[i] = np.sin(k*(gR*np.pi*mdata[1, i])*T**2+fvibtest)
    # fat = vfunc(ta)
    # intvib = fat*a
...
for i in range(data.shape[0]//n):
    if i != 0: idata = np.vstack((idata, data[:,1][n*i:n*(i+1)])) # присоединение прохода

    # fit каждого прохода
    initial_guess = [3000, 2*np.pi/Tg, 0, np.mean(idata[i])] 
    par, cov = curve_fit(sins, mdata[2][n*i:n*(i+1)], idata[i], p0=initial_guess)
    A, w, ph, s = par
    sg = sign(A)
...
# усреднение данных
ndata = np.sum(idata, axis=0)/(data.shape[0]//n)
...
par, cov = curve_fit(sins, mdata[2][:n], ndata, p0=initial_guess)
A, w, ph, s = par
...
plt.figure(1, (10,6))
plt.scatter(mdata[2][:n]*1e-6, ndata, color="blue")
plt.plot(mdata[2][:n]*1e-6, ndata, color="blue", lw =1)
plt.plot(mdata[2][:n]*1e-6, A*np.sin(w*mdata[2][:n]+ph) + s, color="red", ls="--", lw=1.5)
print("signal snr" , A/np.std(ndata-A*np.sin(w*mdata[2][:n]+ph) + s, ddof=1))
...
#plt.xlabel("время интегрирования, с")
